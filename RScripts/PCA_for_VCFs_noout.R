...
setwd("C:/Users/Lauren/Dropbox/Pseudomonas/NoOut")

#Use  your own VCF file
vcf.fn <- "psamples_NoOut_trim.vcf"

#Reformat
snpgdsVCF2GDS(vcf.fn, "psamples_NoOut.gds", method="biallelic.only")

snpgdsSummary("psamples_NoOut.gds")

genofile <-snpgdsOpen("psamples_NoOut.gds")

genofile


#example
genotype = openfn.gds("psamples_NoOut.gds", readonly=TRUE, allow.duplicate = TRUE)
...
ggplot(pc.percent.df)+
  geom_point(aes(x = EV, y = Percents), size = 2)+
...
#PCA 1 and 2
ggplot(poptab_meta)+
  geom_point(aes(x = EV1, y = EV2, color = Strain.type), size = 4)#+
#  geom_text(aes(x = EV3, y = EV1, label = sample.id))

#PCA 1 and 3
ggplot(poptab_meta)+
  geom_point(aes(x = EV1, y = EV3, color = Strain.type), size = 8)#+
#  geom_text(aes(x = EV3, y = EV1, label = sample.id))

#PCA 1 and 4
ggplot(poptab_meta)+
  geom_point(aes(x = EV1, y = EV4, color = Strain.type), size = 8)#+
#  geom_text(aes(x = EV3, y = EV1, label = sample.id))
