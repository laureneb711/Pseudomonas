...
setwd("C:/Users/Lauren/Dropbox/Pseudomonas/SameHouse")

#Use  your own VCF file
vcf.fn <- "psamples_SameHouse_trim.vcf"

#Reformat
snpgdsVCF2GDS(vcf.fn, "psamples_SameHouse.gds", method="biallelic.only")

snpgdsSummary("psamples_SameHouse.gds")

genofile <-snpgdsOpen("psamples_SameHouse.gds")

genofile


#example
genotype = openfn.gds("psamples_SameHouse.gds", readonly=TRUE, allow.duplicate = TRUE)
...
#PCA 1 and 2
ggplot(poptab_meta)+
  geom_point(aes(x = EV1, y = EV2, color = House, shape = Area), size = 6)+
