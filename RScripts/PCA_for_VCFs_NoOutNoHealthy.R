...
setwd("C:/Users/Lauren/Dropbox/Pseudomonas/NoOutNoHealthy")

#Use  your own VCF file
vcf.fn <- "psamples_NoOutNoHealthy_trim.vcf"

#Reformat
snpgdsVCF2GDS(vcf.fn, "psamples_NoOutNoHealthy.gds", method="biallelic.only")

snpgdsSummary("psamples_NoOutNoHealthy.gds")

genofile <-snpgdsOpen("psamples_NoOutNoHealthy.gds")

genofile


#example
genotype = openfn.gds("psamples_NoOutNoHealthy.gds", readonly=TRUE, allow.duplicate = TRUE)
...
#PCA 1 and 2
ggplot(poptab_meta)+
  geom_point(aes(x = EV1, y = EV2, color = House), size = 6)+
